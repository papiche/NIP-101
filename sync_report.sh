...
extract_sync_stats() {
    ...
    local start_time=$(grep "Starting Astroport constellation backfill process" "$log_file" | tail -1 | sed 's/.*\[\([0-9-]* [0-9:]*\)\].*/\1/')
    local end_time=$(grep "Backfill process completed" "$log_file" | tail -1 | sed 's/.*\[\([0-9-]* [0-9:]*\)\].*/\1/')
    
    # Count peers processed
    local total_peers=$(grep "Found.*peers from swarm" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    local success_peers=$(grep "Success:" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    
    # Count events processed
    local total_events=$(grep "Total events collected across all batches:" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    ...
    local hex_count=$(grep "Found.*HEX pubkeys in constellation" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    
    # Count profiles extracted
    local profiles_found=$(grep "Found.*HEX pubkeys with profiles" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    local profiles_missing=$(grep "Found.*HEX pubkeys WITHOUT profiles" "$log_file" | tail -1 | grep -o '[0-9]*' | head -1)
    ...
    # Performance metrics
    local hex_cache_time=$(grep "get_constellation_hex_pubkeys cached:" "$log_file" | tail -1 | grep -o '[0-9]*ms' | head -1)
    local peers_time=$(grep "discover_constellation_peers:" "$log_file" | tail -1 | grep -o '[0-9]*ms' | head -1)
    local batch_scan_time=$(grep "Batch strfry scan for all HEX:" "$log_file" | tail -1 | grep -o '[0-9]*ms' | head -1)
    local parallel_sync_time=$(grep "Parallel full sync:" "$log_file" | tail -1 | grep -o '[0-9]*ms' | head -1)
...
send_sync_report() {
    ...
    if [[ -x "$HOME/.zen/Astroport.ONE/tools/mailjet.sh" ]]; then
        "$HOME/.zen/Astroport.ONE/tools/mailjet.sh" "$captain_email" "$temp_html" "$report_title"
