...
streams {
    backfill_${RANDOM} {
        ...
        filter = { 
            "kinds": [0, 1, 3, 4, 5, 6, 7, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 10000],  # Profiles, text notes, contacts, DMs, deletions, reposts, reactions, videos (short/long), channel creation/metadata/messages/mute (40-44 - NIP-28), file metadata (1063 - NIP-94), comments (1111 - NIP-22), voice messages (1222, 1244 - NIP-A0), user tags (1985 - NIP-32), TMDB enrichments (1986, 30001 - NIP-71 extension), playlists (30005, 10001 - NIP-51), blog, calendar, user statuses (30315 - NIP-38), DID documents (30800 - NIP-101), ORE spaces/meetings (30312-30313), Oracle permits (30500-30503), cookie workflows (31900-31902 - NIP-101 extension), analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
...
EOF
    ...
    local kinds_array="[0, 1, 3, 5, 6, 7, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 10000]"  # Base kinds + videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + user statuses + DID + ORE + Oracle + workflows + analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
    if [[ "$INCLUDE_DMS" == "true" ]]; then
        kinds_array="[0, 1, 3, 4, 5, 6, 7, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 10000]"  # Include DMs + videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + user statuses + DID + ORE + Oracle + workflows + analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
...
execute_backfill_websocket_single_hex() {
    ...
    # Build kinds array based on INCLUDE_DMS setting
    if [[ "$INCLUDE_DMS" == "true" ]]; then
        req_message+='"kinds": [0, 1, 3, 4, 5, 6, 7, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 10000], '  # Include DMs + videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + user statuses + DID + ORE + Oracle + workflows + analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
    else
        req_message+='"kinds": [0, 1, 3, 5, 6, 7, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 10000], '  # Exclude DMs but include videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + user statuses + DID + ORE + Oracle + workflows + analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
    fi
