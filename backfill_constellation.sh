...
streams {
    backfill_${RANDOM} {
        ...
        filter = { 
            "kinds": [0, 1, 3, 4, 5, 6, 7, 8, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 30008, 30009, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 31910, 10000],  # Profiles, text notes, contacts, DMs, deletions, reposts, reactions, badge awards (8 - NIP-58), videos (short/long), channel creation/metadata/messages/mute (40-44 - NIP-28), file metadata (1063 - NIP-94), comments (1111 - NIP-22), voice messages (1222, 1244 - NIP-A0), user tags (1985 - NIP-32), TMDB enrichments (1986, 30001 - NIP-71 extension), playlists (30005, 10001 - NIP-51), profile badges (30008 - NIP-58), badge definitions (30009 - NIP-58), blog, calendar, user statuses (30315 - NIP-38), DID documents (30800 - NIP-101), ORE spaces/meetings (30312-30313), Oracle permits (30500-30503), cookie workflows (31900-31902 - NIP-101 extension), N² Memory (31910 - NIP-101 extension), analytics (10000 - NIP-10000, encrypted/non-encrypted determined by content)
...
EOF
    ...
    local kinds_array="[0, 1, 3, 5, 6, 7, 8, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 30008, 30009, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 30850, 30851, 31900, 31901, 31902, 31910, 10000]"  # Base kinds + badge awards (8 - NIP-58) + videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + profile badges (30008 - NIP-58) + badge definitions (30009 - NIP-58) + user statuses + DID + ORE + Oracle + economic health (30850/30851 - NIP-101 extension) + workflows + N² Memory (31910) + analytics (10000 - NIP-10000)
    if [[ "$INCLUDE_DMS" == "true" ]]; then
        kinds_array="[0, 1, 3, 4, 5, 6, 7, 8, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 30008, 30009, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 30850, 30851, 31900, 31901, 31902, 31910, 10000]"  # Include DMs + badge awards (8 - NIP-58) + videos + files + comments + voice messages + user tags + channels + mute + TMDB enrichments + playlists (30005/10001 - NIP-51) + profile badges (30008 - NIP-58) + badge definitions (30009 - NIP-58) + user statuses + DID + ORE + Oracle + economic health (30850/30851 - NIP-101 extension) + workflows + N² Memory (31910) + analytics (10000 - NIP-10000)
...
execute_backfill_websocket_single_hex() {
    ...
    # Build kinds array based on INCLUDE_DMS setting
    if [[ "$INCLUDE_DMS" == "true" ]]; then
        req_message+='"kinds": [0, 1, 3, 4, 5, 6, 7, 8, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 30008, 30009, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 31910, 10000], '  # Include DMs + N² Memory (31910) + all standard kinds
    else
        req_message+='"kinds": [0, 1, 3, 5, 6, 7, 8, 21, 22, 40, 41, 42, 44, 1063, 1111, 1222, 1244, 1985, 1986, 30001, 30005, 30008, 30009, 10001, 30023, 30024, 30312, 30313, 30315, 30500, 30501, 30502, 30503, 30800, 31900, 31901, 31902, 31910, 10000], '  # Exclude DMs + include N² Memory (31910) + all standard kinds
    fi
